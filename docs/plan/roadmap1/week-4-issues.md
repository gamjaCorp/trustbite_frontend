...
- 등급 사다리(`GET /api/grades`) 연동: `mergeGradeLadder`(서버 문턱값 + 로컬 표현 병합, `src/lib/domain/grade-levels.ts`)까지만 구현됨. `GradeGuideCard`가 아직 이 병합 결과를 안 쓰고 `GRADE_LEVELS` 하드코딩 문턱값을 그대로 그림 — 서버 값이 화면에 반영되려면 `AllGradesTimeline`·`NextStagePanel`에 `ladder` prop 연결 필요. `page.tsx`의 `if (!profile || !grades)`도 분리 안 됨 — 지금은 `/api/grades` 실패 시 폴백 대신 프로필 전체가 에러 화면으로 빠짐.
