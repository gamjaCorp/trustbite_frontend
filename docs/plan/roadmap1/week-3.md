...
| 접근성 | - 접근성 1차 — 텍스트 대비, 클릭 영역 ≥ 44px, `alt` 텍스트 | — | [x] |
|  | - 컴포넌트 함수 위 한 줄 한국어 설명 주석 점검 | — | [x] |
| 레이아웃 | - `/review/new` 데스크톱 2컬럼 — 데스크톱만 사이드바, 모바일 단일 컬럼 | — | [x] |
|  | - 헤더 모바일 — 중요하지 않은 요소 축소/숨김 | — | [x] |
|  | - 탐색 탭 필터 칩 모바일 가로 스크롤 처리 | — | [x] |
| 확인 | - 지도 선택 핀 강조(미니카드 대체) 모바일 표시 확인 | — | [x] |
|  | - 등급 타임라인 모바일 가독성 확인 | — | [x] |
|  | - `text-xs`(12px) 미만 텍스트 없는지 재확인 | — | [x] |
| 검증 | - `pnpm design:check` 통과 + critical 이슈 즉시 수정 | — | [x] |
