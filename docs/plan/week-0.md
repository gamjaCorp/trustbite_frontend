...
- `src/app/me/page.tsx` → `src/app/profile/page.tsx` (커밋 6e92cea)
- [-] `src/app/me/grade/page.tsx` → `src/app/profile/grade/page.tsx` — 스킵: 원본 없음, W1 등급 화면 구현 시 자연 추가
- `src/app/my/page.tsx` → `src/app/my-places/page.tsx` (커밋 6e92cea)
- 빈 `src/app/me/`, `src/app/my/` 삭제 (rename으로 자연 제거)
...
- 프로젝트 전체 grep으로 `'/me'`, `'/me/grade'`, `'/my'` 문자열 사용처 모두 갱신 → 0건
- `'/login?next=/me'` 같은 redirect 경로도 같이 갱신 → 하드코딩 없음 확인
...
- `pnpm dev` — `/profile`, `/my-places` 진입 확인 (사용자 수동 확인)
- 헤더 nav 클릭 동선 확인
- `grep -rn "'/me\|'/my\|\"/me\|\"/my" src/` → 0건
...
- `globals.css` L277 위에 매핑 가이드 주석 박기 — 이미 L277–288에 존재
- 현황 grep — 치환 전 베이스라인 기록 (raw 조합 16건 전부 shadcn ui, app/feature 0건; text-xs 109건; arbitrary hex 0건; hex 8건):
  ```bash
...
- `src/components/common/` — 타이포 + 컬러 (grade-badge, intro-card, trust-score-badge, trust-score-sheet)
...
- `src/components/features/my-restaurant/**` — 타이포 + 컬러 (+ `var(--score-*)` 직접 참조 → `text-score-*` 시맨틱 교체)
- `src/components/features/user-profile/**` — 타이포 + 컬러
- `src/components/features/auth/**` — text-xs 0건, 컬러 위반 없음
- `src/app/**` 페이지 레벨 타이포 (login, my-places, onboarding, restaurant/[id], signin)
- `text-xs` 사용처 중 카드 본문/메타에 해당하는 곳 `text-body-2`로 승격 (점수 행 3건 + 리뷰 코멘트 1건)

**점검**

- `pnpm lint && npx tsc --noEmit` — 그린
- `grep -rn "#[0-9a-fA-F]\{3,6\}\b" src/components src/app --include="*.tsx" | grep -v "Google\|브랜드"` → restaurant-pin SVG 4건 잔존 (Day 2에서 currentColor 처리)
...
- `common/Header.tsx` → `core/header.tsx`
- `common/grade-badge.tsx` → `core/grade-badge.tsx`
- `common/intro-card.tsx` → `core/intro-card.tsx`
- `common/rank-card-skeleton.tsx` → `core/rank-card-skeleton.tsx`
- `common/trust-score-badge.tsx` → `core/trust-score-badge.tsx`
- `common/trust-score-sheet.tsx` → `core/trust-score-sheet.tsx`
- `@/components/common/*` import 전체 → `@/components/core/*`로 일괄 수정
- 빈 `common/` 삭제
